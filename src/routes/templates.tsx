  ...
  return (
    <div className="px-6 md:px-10 py-6 md:py-8 max-w-6xl">
      <div className="flex items-end justify-between mb-6">
        <div>
          <h1 className="font-display text-3xl md:text-4xl">Component Designer</h1>
          <p className="text-muted-foreground text-sm mt-1">Define types and custom fields for the components in your games.</p>
        </div>
      ...
      {(templates?.length ?? 0) === 0 ? (
        <div className="border border-dashed border-border rounded-xl p-12 text-center bg-card/40">
          ...
          <h3 className="font-display text-2xl mt-4">No templates yet</h3>
          <p className="text-sm text-muted-foreground mt-1">Cards, chips, dice — define each kind once, then reuse across games.</p>
        </div>
      ) : (
          ...
          {templates!.map((t) => (
            <button key={t.id} onClick={() => setEditing(t)} className="text-left bg-card border border-border rounded-xl p-5 hover:border-accent transition-colors shadow-[var(--shadow-card)]">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                ...
                {t.fields.length === 0 && <span className="text-xs text-muted-foreground">No fields</span>}
              </div>
              {t.kind === "card" && (
                <div className="mt-3 text-xs text-muted-foreground">
                  {t.cardWidthMm ?? 63}×{t.cardHeightMm ?? 88} mm · {t.twoSided ? "Two-sided" : "Single-sided"}
                </div>
              )}
            </button>
          ))}
...
function TemplateEditor({ initial, onClose }: { initial?: ComponentTemplate; onClose: () => void }) {
  ...
  const remove = async () => {
    if (!initial) return;
    if (!confirm("Delete template? Existing components keep their data but lose schema.")) return;
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/40 backdrop-blur-sm p-4" onClick={onClose}>
      <form onClick={(e) => e.stopPropagation()} onSubmit={save} className="bg-card w-full max-w-2xl rounded-xl border border-border shadow-[var(--shadow-deep)] p-6 max-h-[92vh] overflow-auto">
        <h2 className="font-display text-2xl mb-4">{initial ? "Edit template" : "New template"}</h2>
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-display text-lg">Fields</h3>
          ...
          {fields.length === 0 && <div className="text-sm text-muted-foreground border border-dashed border-border rounded-md p-4 text-center">No fields yet.</div>}
          <div className="space-y-2">
            {fields.map((f) => (
              <div key={f.id} className="grid grid-cols-12 gap-2 items-center">
                <input value={f.name} onChange={(e) => updField(f.id, { name: e.target.value })} className={`${inputCls} col-span-5`} placeholder="Field name" />
                ...
                {f.type === "select" ? (
                  ...
                  <div className="col-span-3 text-xs text-muted-foreground">{f.type === "image" ? "Image URL/data" : ""}</div>
                )}
        ...
        <div className="mt-6 flex justify-between gap-2">
          <div>
            {initial && (
              <button type="button" onClick={remove} className="text-sm text-destructive hover:underline">Delete template</button>
            )}
          </div>
