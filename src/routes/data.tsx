...
function DataPage() {
  ...
  const exportAll = async () => {
    ...
    setMsg(`Exported ${games.length} games, ${templates.length} templates, ${components.length} components.`);
  };
  ...
  const importFile = async (file: File) => {
    try {
      ...
      if (data.app !== "meeple-vault") throw new Error("Not a Meeple Vault file");
      ...
      setMsg(`Imported ${data.games?.length ?? 0} games, ${data.templates?.length ?? 0} templates, ${data.components?.length ?? 0} components.`);
    } catch (e: any) {
  ...
  return (
    <div className="px-6 md:px-10 py-6 md:py-8 max-w-3xl">
      <h1 className="font-display text-3xl md:text-4xl">Import / Export</h1>
      <p className="text-muted-foreground text-sm mt-1">Back up your collection or move it between machines.</p>

      <div className="mt-8 grid md:grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-xl p-6 shadow-[var(--shadow-card)]">
          <Download className="h-6 w-6 text-accent" />
          <h2 className="font-display text-2xl mt-3">Export to JSON</h2>
          <p className="text-sm text-muted-foreground mt-1">All games, templates, components and embedded images.</p>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-[var(--shadow-card)]">
          ...
          <h2 className="font-display text-2xl mt-3">Import JSON</h2>
          <p className="text-sm text-muted-foreground mt-1">Merges with existing data — same IDs are overwritten.</p>
